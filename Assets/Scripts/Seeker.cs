...
public class Seeker : MonoBehaviour
{
	...
	private GameObject[] obsList = null;
	...
	public void Start ()
	{
		...
		obsList = GameObject.FindGameObjectsWithTag("Obstacle");
	...
	private void CalcSteeringForce ()
	{
		...
		{
			if (Vector3.Distance(transform.position, obsList[i].transform.position) < 100)
			{
				steeringForce += avoidWt * steering.AvoidObstacle(obsList[i], avoidDist);
			}
		}
		//Debug.Log(Vector3.Distance(transform.position, target.transform.position));

		if (Vector3.Distance(transform.position, target.transform.position) < 27)
		{
			transform.position = startPos;
		}
		else
